...
| `training/pose_train.py` | Train the TMP pose model (YOLO + SAGA backbone) |
...
# Inference
python inference/inference.py --run runs/train/expN --mouse-keypoints path/to/mouse/ --tail-keypoints path/to/tail/ --output results.csv
...
# Held-out validation
python inference/validation.py --run runs/train/expN
```
